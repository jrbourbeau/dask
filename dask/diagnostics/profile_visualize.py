...
def get_colors(palette, funcs):
    ...
    unique_funcs = list(sorted(toolz.unique(funcs)))
    ...
    palette = list(toolz.unique(palette))
...
def plot_tasks(results, dsk, palette="Viridis", label_size=60, **kwargs):
    ...
    **kwargs
    ...
    if results:
        ...
        id_group = toolz.groupby(itemgetter(4), results)
        timings = dict(
...
def plot_cache(
    ...
    **kwargs
    ...
    if results:
        ...
        tics = list(sorted(toolz.unique(starts + ends)))
        groups = toolz.groupby(lambda d: pprint_task(d[1], dsk, label_size), results)
        data = {}
        for k, vals in groups.items():
            ...
            data[k] = [0] + list(
                toolz.accumulate(add, toolz.pluck(1, sorted(cnts.items())))
            )
